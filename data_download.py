...
_TRAIN_DATA_SOURCES = [ # 3个数据集构成训练集
    {
        "url": "http://data.statmt.org/wmt17/translation-task/" # 下载网址
               "training-parallel-nc-v12.tgz",
        "input": "news-commentary-v12.de-en.en", # 源语言
        "target": "news-commentary-v12.de-en.de", # 目标语言
    },
    {
...
_EVAL_DATA_SOURCES = [ # 1个数据集构成验证集，同样的数据信息
    {
...
def find_file(path, filename, max_depth=5): # 在path中找文件filename，找到则返回该文件绝对路径
...
def get_raw_files(raw_dir, data_source):
  ...
  """
  raw_files = { # 一个字典，两个键值对，键为inputs的，值为若干个文件路径，每个路径是一个源语言文件；键为targets的，同为若干个文件路径
      "inputs": [],
      "targets": [],
  }  # keys
...
def compile_files(raw_dir, raw_files, tag): # 将多个文件合并成一个文件，源语言一个文件，目标语言一个文件，还是存在raw_dir目录下
...
# Data preprocessing
###############################################################################
def encode_and_save_files( # 将raw_files里面的源语言文件和目标语言文件转成TFRecord格式
  ...
  filepaths = [shard_filename(data_dir, tag, n + 1, total_shards) # 获取分片之后的文件名们
               for n in range(total_shards)]

  if all_exist(filepaths): # 判断文件是否已经存在了
  ...
  tmp_filepaths = [fname + ".incomplete" for fname in filepaths] # 在要存成的文件名后面加个未完成的标志
  writers = [tf.python_io.TFRecordWriter(fname) for fname in tmp_filepaths] # 将所有的写指针构成一个数组
  counter, shard = 0, 0
  for counter, (input_line, target_line) in enumerate(zip( # 从源语言文件和目标语言文件中读一行，往输出文件中写一个样本
  ...
  for tmp_name, final_name in zip(tmp_filepaths, filepaths): # 存完之后，将文件名的未完成标志去掉
    tf.gfile.Rename(tmp_name, final_name)

  tf.logging.info("Saved %d Examples", counter + 1)
  return filepaths # 将存下来的文件名返回
...
def main(unused_argv):
  ...
  make_dir(FLAGS.raw_dir) # 存放下载数据
  make_dir(FLAGS.data_dir) # 存放转换格式之后的数据
  ...
  # 给定下载保存路径和数据来源，得到# 一个字典，两个键值对，键为inputs的，值为若干个文件路径，每个路径是一个源语言文件；键为targets的，同为若干个文件路径
  ...
  subtokenizer = tokenizer.Subtokenizer.init_from_files( # 从所有文件中，找出所有token，找出所有字符，然后找出所有subtoken
  ...
  # compile_files() 返回两个文件，源语言文件和目标语言文件
  compiled_train_files = compile_files(FLAGS.raw_dir, train_files, _TRAIN_TAG) # 对于训练数据，将源语言的几个文件合并成一个文件，同理于目标语言，还是存在raw_dir目录下
  compiled_eval_files = compile_files(FLAGS.raw_dir, eval_files, _EVAL_TAG) # 同理于训练数据，处理验证数据

  # Tokenize and save data as Examples in the TFRecord format.
  tf.logging.info("Step 4/4: Preprocessing and saving data")
  train_tfrecord_files = encode_and_save_files( # 这大概是要转成TFRecord格式了，存到data_dir目录下面
  ...
  for fname in train_tfrecord_files: # 将每个文件内部进行一下shuffle
    shuffle_records(fname)


def define_data_download_flags(): # 设置可传参数
