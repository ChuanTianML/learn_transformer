...
class Subtokenizer(object):
  ...
  def init_from_files(
    ...
    """
    if reserved_tokens is None:
      reserved_tokens = RESERVED_TOKENS # 有一些保留token，例如开始符，停止符

    if tf.gfile.Exists(vocab_file):
      tf.logging.info("Vocab file already exists (%s)" % vocab_file)
    else:
      tf.logging.info("Begin steps to create subtoken vocabulary...")
      token_counts = _count_tokens(files, file_byte_limit) # 先统计token，返回一个字典，<token:count>
      alphabet = _generate_alphabet_dict(token_counts) # 返回的应该是个集合，里面是所有token和字符
      subtoken_list = _generate_subtokens_with_target_vocab_size( # 应该是生成subtoken，然后跟其他token合并，最终返回的是list类型
...
def _list_to_index_dict(lst): # 将list转为一个字典，item：index
...
def _generate_alphabet_dict(iterable, reserved_tokens=None): # 这一步仅仅是获得一个字符表，还不知道对中文怎么处理，是否是按照字切分
  ...
  if reserved_tokens is None:
    reserved_tokens = RESERVED_TOKENS
  alphabet = {c for token in iterable for c in token} # 从统计出的token里面获取字符
  alphabet |= {c for token in reserved_tokens for c in token} # 从保留token集合里面获取字符
  alphabet |= _ESCAPE_CHARS  # Add escape characters to alphabet set. # 还有一些保留字符
  return alphabet # 最后，返回的应该是个集合，里面是所有token和字符
...
def _generate_subtokens(
  ...
  subtoken_list = reserved_tokens + list(alphabet) # 初始化subtoken_list，包含所有token
